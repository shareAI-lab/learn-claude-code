...
if __name__ == "code":
    ...
    PERMISSION_RULES = [
        ...
        {"tools": ["bash"],
         "check": lambda args: "pip install" in args.get("command", ""),
         "message": "Installing packages changes the environment"},
    ]
    ...
    def agent_loop(messages: list):
        while True:
            ...
            for block in response.content:
                ...
                if not check_permission(block):
                    print("\n[Permission] 用户拒绝或权限不通过，工具不会执行。")
                    denied_message = (
                        "Permission denied. Installing packages is not allowed in this session. "
                        "Do not try pip install again; explain the limitation to the user."
                    )
                    print(f"[Permission] 返回给模型：{denied_message}\n")
