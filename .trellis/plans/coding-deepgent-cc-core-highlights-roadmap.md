...
| H12 | Fork/cache-aware subagent execution | implemented | explicit `run_fork` foreground/background surface, fork continuity metadata, background fork reuse, stop/cancel contract, and resume/path hardening on one local slice; provider-specific cache tuning remains out of scope | `subagents`, `runtime`, `compact` | local completion pack is complete; implicit fork entry and provider-specific cache internals remain intentionally out of scope |
