"""M3-2: 退出总结写 .oaic/MEMORY.md 测试。"""
from __future__ import annotations

from oai_code.config import load_config
from oai_code.memory import summarize_to_memory
...
def test_empty_conversation(tmp_path, monkeypatch):
    ...
    assert not (tmp_path / ".oaic" / "MEMORY.md").exists()
...
def test_second_append_no_duplicate_header(tmp_path, monkeypatch):
    ...
    text = (tmp_path / ".oaic" / "MEMORY.md").read_text()
    # header 只出现一次
