...
All notable changes to oai-code are documented here. Format loosely follows
...
**M0 — Core skeleton**
- CLI entry `oaic` with REPL + one-shot (`-p`) + resume (`--resume`) modes.
...
- Four-level config loading: env → user → project → `--config` / `OAIC_CONFIG` → CLI flags. Pydantic validation, fail-fast.
...
- Persistent `TaskCreate` / `TaskGet` / `TaskUpdate` / `TaskList` under `.oaic/tasks/`; cascade unblock on completion; `/tasks` slash.
...
- microcompact (evict large tool_result to `.oaic/blobs/`) and auto-compact (summarize with `roles.summarize` → `.oaic/transcripts/`).
- Role-based models: `roles.main` / `roles.summarize` / `roles.subagent`.

**M2 — Long-running workflows**
- Session persistence: `.oaic/sessions/<id>.jsonl` appended per turn; redacted before write; `--resume <id|latest>`; `--list-sessions`; `/sessions` / `/resume` / `/save`.
...
- Exit-time summary: `/exit-summary` or `/quit --summary` → `.oaic/MEMORY.md` (append-only, never overwrites `CLAUDE.md`).
...
- Autonomy (s11): teammates poll `.oaic/tasks/` during IDLE and auto-claim unblocked tasks; identity re-injection after compact.
...
- Worktree isolation: `EnterWorktree` / `ExitWorktree` / `WorktreeStatus` back a git worktree under `.oaic/worktrees/<name>/`; `workspace_root` swaps via a PrivateAttr (no `os.chdir`); `state.json` enables resume across restarts.
...
- First public release version `0.1.0`; PyPI name `oai-code` confirmed available at time of publish.

### Notes

- This is a pre-1.0 release. APIs in `src/oai_code/` may still change without deprecation. The tool names and schemas in `docs/TOOLS.md` are the stable contract for LLM interaction.
