...
def _run_enter(cfg: Config, name: str | None = None, branch: str | None = None) -> str:
    ...
    worktree_dir = origin / ".oaic" / "worktrees" / name
    if worktree_dir.exists():
        return f"Error: worktree path already exists: {worktree_dir}"

    branch_name = branch or f"oaic/{name}"
...
def register_worktree(registry: ToolRegistry, cfg: Config) -> None:
    registry.register(
        Tool(
            name="EnterWorktree",
            description=(
                "Create a git worktree under .oaic/worktrees/<name>/ on a new "
            ...
            input_schema={
                ...
                "properties": {
                    ...
                    "branch": {
                        "type": "string",
                        "description": "Branch name for the new worktree. Default: oaic/<name>",
                    },
                },
            },
