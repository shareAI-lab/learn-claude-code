...
| # | 需求 | 期望 | 验证方式 | 结果 |
|---|------|------|---------|------|
| M5-5.1 | 包名 `oai-code` 可用 | PyPI 404 | `httpx.get pypi/pypi/oai-code/json` | ✅ |
| M5-5.2 | 版本号 0.1.0 | pyproject + `__init__.py` 一致 | `oaic --version` | ✅ |
| M5-5.3 | CHANGELOG.md 列出 M0-M5 | 可读 | 人工读 | ✅ |
| M5-5.4 | README 英文摘要 | 顶部 English summary 段 | 人工读 | ✅ |
| M5-5.5 | `uv build` 产出 wheel | 0.1.0 版本 | `ls dist/` | ✅ |
| M5-5.6 | wheel 含 entry_points | oaic → oai_code.cli:main | `unzip -p *.whl entry_points.txt` | ✅ |
...
```
src/oai_code/tools/worktree.py     ≈ 230 行
src/oai_code/config/models.py      +10 行 (PrivateAttr)
src/oai_code/ui/repl.py            +60 行 (耗时 + Todo 富渲染)
src/oai_code/cli.py                +20 行 (worktree 集成)
...
# 1. 版本号正确
uv run oaic --version                             # oai-code 0.1.0

# 2. Worktree
uv run oaic
# oaic > 用 EnterWorktree 建叫 feature-x 的 worktree
# oaic > /tools | head            # 能看到 EnterWorktree/ExitWorktree
# oaic > WorktreeStatus           # 显示当前在 worktree
# oaic > ExitWorktree action=keep  # 退出但保留分支
...
# (可选)用一个干净 Python venv 装 wheel,跑 oaic --help

# 5. PyPI 占用查
uv run python -c "
import httpx
r = httpx.get('https://pypi.org/pypi/oai-code/json', timeout=10)
...
**最终结论**：**✅ M5 验收通过。oai-code 0.1.0 已具备对外发布条件，待维护者按 `docs/PUBLISH.md` 推送 PyPI**。
