...
|---|------|---------|-----------|-------------|------|
| M1-1.1 | 读取项目根 CLAUDE.md | 进入 system prompt | `test_load_basic` | 在项目根建 `CLAUDE.md`→ 启动 oaic 问 "你记忆里有什么" | ✅ |
| M1-1.2 | 文件不存在不报错 | 静默跳过 | `test_load_missing_returns_none` | 删除 `CLAUDE.md` 启动无异常 | ✅ |
| M1-1.3 | `~/` 展开 | 正确解析到 HOME | `test_home_expand` | 建 `~/.oaic/CLAUDE.md` 启动后可被引用 | ✅ |
...
| # | 需求 | 期望行为 | 自动化测试 | 手动验证方法 | 结果 |
|---|------|---------|-----------|-------------|------|
| M1-3.1 | TaskCreate 落盘 `.oaic/tasks/task_N.json` | 文件存在 | `test_create_and_get` | 让模型建一个任务,`ls .oaic/tasks/` | ✅ |
...
| M1-4.6 | 子 agent 返回文本总结 | 父 agent 收到字符串 | 真实端到端 | `oaic -p "派子 agent 找所有 register_ 函数"` | ✅ |
...
| M1-5.3 | skills_dirs 顺序 = 优先级 | 同名前者胜出 | `test_discover_multiple_dirs_first_wins` | 在 `./skills/x/` 和 `~/.oaic/skills/x/` 各放一份不同的 SKILL.md | ✅ |
...
| # | 需求 | 期望行为 | 自动化测试 | 手动验证方法 | 结果 |
|---|------|---------|-----------|-------------|------|
| M1-6.1 | microcompact 保留最近 N 条 tool_result | 老的才外置 | `test_microcompact_evicts_old_large` | 看 `.oaic/blobs/*.txt` 产出 | ✅ |
...
| M1-6.7 | transcripts 落盘 | `.oaic/transcripts/*.jsonl` | `test_auto_compact_produces_summary` | 跑 `/compact` 后 `ls .oaic/transcripts/` | ✅ |
...
```
src/oai_code/memory/loader.py         ≈ 90  行
src/oai_code/tools/todo.py            ≈ 100 行
src/oai_code/tools/tasks.py           ≈ 200 行
src/oai_code/tools/subagent.py        ≈ 130 行
src/oai_code/tools/skills.py          ≈ 110 行
src/oai_code/tools/compact_tool.py    ≈ 40  行
src/oai_code/context/compact.py       ≈ 140 行
src/oai_code/config/models.py         +60  行(Roles*)
src/oai_code/agent/loop.py            +15  行(summarize_llm)
src/oai_code/cli.py / ui/repl.py      +~80 行(串接与 slash)
-----------------------------------------------
...
- [x] 所有新建路径（`.oaic/blobs/` / `.oaic/transcripts/` / `.oaic/tasks/`）均自动 `mkdir`
...
# 1. 基础工具仍然 OK
uv run oaic -p "一句话总结 README.md"

# 2. Memory 生效
echo "# Project Memory\nProject is oai-code, Python CLI." > CLAUDE.md
uv run oaic -p "项目是什么?"   # 应引用 CLAUDE.md 内容
rm CLAUDE.md

# 3. 持久 Task
uv run oaic -p "创建一个 task:写 M1 验收报告"
cat .oaic/tasks/task_*.json

# 4. Subagent
uv run oaic -p "派 Explore 子 agent 列出 src/oai_code/tools 下所有 .py 文件"

# 5. Compact
printf "你好\n/compact\n/quit\n" | uv run oaic   # 应打印 compacted: N → M

# 6. Roles 切换
uv run oaic --provider fenbi-mini -p "你是什么模型"
uv run oaic --provider fenbi-sonnet -p "你是什么模型"
