...
|---|------|---------|-----------|-------------|------|
| M0-A.1 | `uv run oaic --help` | 列出全部 CLI flag | — | 命令执行后看输出 | ✅ |
| M0-A.2 | `uv run oaic` 进入 REPL | 横幅 + `oaic >` 提示符 | — | 直接启动 | ✅ |
| M0-A.3 | `oaic -p "..."` 单次模式 | 输出后退出，退出码 0 | — | `uv run oaic -p "hi"` | ✅ |
...
| M0-B.5 | `memory_files` 默认含用户级 | 列表含 `~/.oaic/CLAUDE.md` | `test_memory_files_default_includes_user_level` | — | ✅ |
| M0-B.6 | `mcp_servers` 按 key 深合并 | user/project 合并不互相覆盖 | `test_deep_merge_mcp_servers` | — | ✅ |
| M0-B.7 | 流式工具调用识别 | 模型 tool_calls 被 dispatcher 执行 | — | `oaic -p "读 README.md"` | ✅ 真实 fenbi 调用 |
...
| M0-C.3 | Read 二进制占位 | `[binary or too large, N bytes]` | `test_read_binary_placeholder` | `oaic -p "读 img.png"` | ✅ |
...
| M0-C.7 | Bash deny-list（`sudo` 等） | 返回 `Error: command blocked` | `test_bash_ok_and_deny` | `oaic -p "用 Bash 跑 sudo ls"` | ✅ |
| M0-C.8 | Bash exit code != 0 | 输出末尾 `[exit code: N]` | `test_bash_exit_code` | — | ✅ |
| M0-C.9 | Glob 按 mtime 倒序 | 新文件在前 | `test_glob` | — | ✅ |
| M0-C.10 | Grep content 模式返回行 | `def foo():` 出现在输出里 | `test_grep_content` | `oaic -p "找 register_ 开头函数"` | ✅ |
...
```
pyproject.toml                       42 行
src/oai_code/cli.py                  ~110 行
src/oai_code/config/{loader,models}  ~170 行
src/oai_code/llm/{client,providers}  ~180 行
src/oai_code/agent/{loop,dispatcher,system_prompt} ~290 行
src/oai_code/tools/{builtin,registry,safety}       ~420 行
src/oai_code/ui/repl.py              ~180 行
tests/                               ~500 行
-----------------------------------------------
...
# 2. 单次模式
uv run oaic -p "用一句话描述 README.md 的内容"

# 3. 工具链真跑
uv run oaic -p "用 Grep 找出所有 register_ 开头的函数定义"
