...
|---|------|---------|-----------|-------------|------|
| M2-1.1 | 启动默认创建新 session | `.oaic/sessions/<id>.jsonl` 文件存在 | `test_new_session_creates_file` | 启动 oaic 后 `ls .oaic/sessions/` | ✅ |
...
| M2-1.4 | `load()` 完整恢复 messages | 从 jsonl 还原成 list | `test_load_roundtrip` | `oaic --resume <id>` 能继续对话 | ✅ |
| M2-1.5 | `--resume` 不存在的 id 报错 | stderr 报 not found | `test_load_missing_raises` | `oaic --resume bogus-id` | ✅ |
| M2-1.6 | `latest_id` / `list_ids` 按 mtime 倒序 | 最新的在前 | `test_list_ids_sorted_by_mtime` / `test_latest_id` | — | ✅ |
| M2-1.7 | `--list-sessions` 打印列表 | 含 msg 数与首条 user 输入 | — | `oaic --list-sessions` | ✅ |
| M2-1.8 | `auto_save=false` 时不落盘 | append_new_messages 返回 0 | `test_auto_save_false_no_write` | `oaic --no-save` 后 `.oaic/sessions/` 无新文件 | ✅ |
...
```
src/oai_code/session/store.py        ≈ 125 行
src/oai_code/tools/background.py     ≈ 140 行
src/oai_code/mcp/client.py           ≈ 195 行
src/oai_code/ui/repl.py              +~120 行（slash + 辅助方法）
src/oai_code/cli.py                  +~60 行（串接）
src/oai_code/agent/loop.py           +~20 行（bg_manager drain）
-----------------------------------------------
...
- [x] 未破坏 M0/M1 已有测试
- [x] `.env` / `.oaic/sessions/` / `.oaic/blobs/` 均在 `.gitignore`（`.oaic/` 一条覆盖）
- [x] 所有新路径自动 `mkdir`
- [x] 无硬编码 API key
...
uv run oaic -p "记住数字 42"
uv run oaic --list-sessions     # 应看到刚才的 session
uv run oaic --resume latest -p "我告诉你什么数字"  # 模型应答 42
...
# 3. MCP 缺配置时不崩
uv run oaic -p "/mcp"   # 或直接启动 REPL 输 /mcp
