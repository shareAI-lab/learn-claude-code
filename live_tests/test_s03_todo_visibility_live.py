...
def test_s03_terminal_plan_is_visible_after_real_llm_generates_todo_args() -> None:
    ...
    这里不用自由代理循环，而是：
    1. 用真实 OpenAI 模型绑定真实 `todo_tool` schema；
    2. 强制 `tool_choice='required'`，确保模型必须产出 todo 参数；
    3. 用这些真实参数调用 `s03.todo(...)`；
    ...
    todo_call = response.tool_calls[0]
    assert todo_call["name"] == "todo"

    command = s03.todo(
        todo_call["args"]["items"],
    ...
    assert "总结 s03 todo 功能是否在终端可见" in terminal_text
