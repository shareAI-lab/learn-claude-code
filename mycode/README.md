...
进入后就像这样对话：
```
mycode provider=fenbi model=pa/gpt-5.4  (/help for commands, Ctrl-D to quit)
mycode > 帮我看下 README.md 说了什么
assistant 这个项目是 mycode,一个基于 OpenAI 兼容协议的命令行编码 Agent ...
mycode > /quit
```

其他使用方式：
```bash
uv run mycode --provider fenbi-sonnet      # 用另一个 profile 启动 REPL
uv run mycode -p "总结一下 README.md"      # 单次执行,跑完就退出
uv run mycode --resume latest              # 恢复最近一次会话
uv run mycode --list-sessions              # 列出所有保存的会话
```
...
> 在 `mycode >` 提示符后敲一句话按回车，到屏幕上看到 `assistant` 开始说话，中间发生了什么？

典型场景：
```
mycode > 帮我看下 README.md 说了什么
assistant 这个项目是 mycode,一个 ...
mycode >
```
...
| 英文 | 中文 | 作用 |
|------|------|------|
| **REPL** | 交互式命令行 / 对话循环 | Read-Eval-Print-Loop，你看到的 `mycode >` 提示符就是它 |
...
| **session** | 会话 | 一次 REPL 从启动到退出之间的完整对话，落盘到 `.mycode/sessions/<id>.jsonl` |
...
```mermaid
flowchart TD
    A[在 mycode ＞ 处输入<br/>'帮我看下 README.md'] --> B[1.REPL 读取整行输入]
    ...
    K --> L[8.Session 追加写盘<br/>.mycode/sessions/id.jsonl<br/>脱敏后]
    L --> E
...
下面把每一步用人话讲一遍，读完你就知道在 `mycode >` 里敲完一句话按下回车之后究竟发生了什么。

**第 1 步 · REPL 读取整行输入**
程序入口是 `src/mycode/cli.py::main`。默认进入 REPL（交互式命令行）模式，由 `prompt_toolkit` 在 `mycode >` 处等你按回车；按下回车后拿到一整行字符串。（另外还有两种非交互模式：`mycode -p "..."` 单次执行后退出、`mycode --resume <id>` 恢复旧会话。三种模式之后的流程完全一样。）

**第 2 步 · 组装 system prompt（系统提示词）**
启动时一次性做：四级合并配置（env → user → project → `--config` → CLI flag）→ 按 `memory_files` 读项目根 `CLAUDE.md` / `AGENTS.md` / `~/.mycode/CLAUDE.md`，并展开里面的 `@other.md` 引用 → 扫描 `skills/`（技能）目录，把 skill 的名字和一句描述（**不是正文**）注入 prompt。到这一步我们有了 `state.messages = [system, user]`（消息数组：一条系统消息 + 一条用户消息）。

**第 3 步 · 每轮循环开头预处理**
Agent Loop（Agent 主循环）每轮调 LLM **之前**要做三件事：
- `microcompact`（微压缩）：超过最近 3 条的大 tool_result（工具执行结果，>4 KiB）外置到 `.mycode/blobs/<hash>.txt`，内存里替换为提示占位。原始文件仍在磁盘上，模型想看可以用 Read 工具找回。
- 估算 token 是否超过 `context_window × 75%`，超了就调 `roles.summarize`（摘要角色，通常配便宜小模型如 `fenbi-mini`）做 `auto-compact`（自动压缩）：把历史压成一条 summary + 保留最近 6 条消息。完整 transcript（对话记录）落盘到 `.mycode/transcripts/`。
...
```
然后 `SessionStore.append_new_messages` 把这一轮新增的消息追加写入 `.mycode/sessions/<session-id>.jsonl`（会话持久化文件），写入前过一次 `redact`（脱敏）：把 Bearer token / `sk-*` 前缀密钥 / `api_key` 字段都替换为 `[REDACTED]`。
...
6. **Session（会话）每轮落盘**（第 8 步）：不是退出时一次性写。异常崩溃也不丢数据，`mycode --resume latest` 可继续上次对话。

详见 [`docs/DESIGN.md`](./docs/DESIGN.md) §5（核心数据流）和 §4（各模块职责）。

## 配置

最小配置示例（`~/.mycode/settings.json`）：
