...
**一句话**：mycode 是一个 Python CLI 编码 Agent，底层通过 OpenAI 兼容协议调用任意 LLM（OpenAI / DeepSeek / Qwen / OpenRouter / Ollama / vLLM 等），对外呈现与 Claude Code 类似的交互体验和工具集。
...
| 打包 | `uv` / `pyproject.toml` | 单命令安装，可 `pipx install mycode` |
...
┌──────────────────────────────────────────────────────────────┐
│                         REPL / CLI 入口                         │
│  mycode (交互)  │  mycode -p "..." (单次)  │  mycode /slash-cmd       │
└────────────────────────┬─────────────────────────────────────┘
                         │
┌────────────────────────▼─────────────────────────────────────┐
│                       Agent Orchestrator                       │
...
│(OpenAI │   │ + Dispatcher│ │(microcompact│ │(.mycode/)   │
│  SDK)  │   │           │  │ + auto)     │ │           │
└────────┘   └─────┬─────┘  └────────────┘ └────────────┘
                   │
      ┌────────────┼─────────────────────────────┐
...
| `Compact` | s06 | 手动压缩上下文 |
| `TaskCreate/Update/List/Get` | s07 | 持久化任务系统（`.mycode/tasks/`） |
...
### 4.3 `mycode/context/` — 上下文管理
- **microcompact**：每轮 LLM 调用前的量化规则
  - 保留最近 **N=3** 轮完整 tool_result 原文
  - 更早的 tool_result：体积 > **4KB** 时落盘到 `.mycode/blobs/<hash>.txt`，in-memory 替换为 `[evicted: Read .mycode/blobs/<hash>.txt]`
  - 所有 user 文本消息与 assistant 消息（含 tool_calls 结构）**永不删除**
- **auto-compact**：超过阈值时 LLM 摘要 + 落盘完整 transcript 到 `.mycode/transcripts/`
- 阈值按**模型 context window 百分比**（默认 75%），而非硬编码 100k

### 4.4 `mycode/session/` — 会话与持久化
- 项目根目录下自动创建 `.mycode/`：
...
- 支持 `mycode --resume <session-id>` 恢复

### 4.5 `mycode/skills/` — Skills 机制
- 兼容 Claude Code 的 `SKILL.md` frontmatter 格式
- 启动时只加载 **name + description**，模型按需 `LoadSkill` 读正文
- 同时扫描 **项目级** `./skills/` 和 **用户级** `~/.mycode/skills/`

### 4.6 `mycode/memory/` — 记忆文件
- 读取 `config.memory_files` 指定的所有文件（默认见 `CONFIG.md §3.4`）
- 项目级文件按数组顺序读取；用户级 `~/.mycode/CLAUDE.md` 固定**追加在数组末尾**（见 CONFIG.md 默认值）
- 按顺序拼到系统提示开头（与 Claude Code 语义一致）

### 4.7 `mycode/mcp/` — MCP 客户端
- 通过 `.mycode/settings.json` 配置 MCP servers
...
- debug 日志默认脱敏；`--debug-raw` flag 才输出原文，仅写到 `.mycode/debug/` 且不随 session 走
...
- **退出时记忆总结**（显式触发）：
  ...
  - 写入位置：**追加**到 `.mycode/MEMORY.md`（不动用户手写的 `CLAUDE.md`）
  - 条目格式：带时间戳的 Markdown 块，便于后期人工审核删除
  - 默认行为：不主动触发，需用户显式指令
- 发布到 PyPI（包名 `mycode`、CLI `mycode`；发布前需检索 PyPI 占用，结论**待确认**）
