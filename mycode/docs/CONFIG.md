# mycode 配置规格（CONFIG.md）

> 四级优先级（高 → 低）：**CLI flag > 项目 `.mycode/settings.json` > 用户 `~/.mycode/settings.json` > 环境变量**。
...
2. 加载 `~/.mycode/settings.json` → 覆盖 EnvDefaults（**深合并**：dict 逐键覆盖，list/scalar 整体替换）
3. 加载 `<cwd>/.mycode/settings.json` → 深合并覆盖
4. 加载 `--config <path>` 或 `$MYCODE_CONFIG` 指向的文件 → 深合并覆盖（同规则；两者同时存在时 `--config` 胜出；均不设则跳过）
5. 应用 CLI flag → 顶层字段整体替换
6. Pydantic 校验 → 任一非法字段直接报错退出，不尝试"尽量运行"

**`--config` / `MYCODE_CONFIG` 的层级定位**：位于 **项目级 settings < config 文件 < CLI flag** 之间。典型用法是临时叠加一个 profile（如 `--config ci.json` 跑 CI），但 CLI flag 始终是"最终话语权"。
...
```json
{
  "$schema": "https://mycode.dev/schema/v1.json",
  ...
  "memory_files": ["CLAUDE.md", "AGENTS.md", ".mycode/MEMORY.md", "~/.mycode/CLAUDE.md"],
...
| 字段 | 默认 | 说明 |
|------|------|------|
| `skills_dirs` | `["./skills", "~/.mycode/skills"]` | 扫描顺序即优先级，同名时前者覆盖后者 |
| `memory_files` | `["CLAUDE.md", "AGENTS.md", ".mycode/MEMORY.md", "~/.mycode/CLAUDE.md"]` | 按数组顺序读取并拼接到 system prompt；前 3 项为项目级（基于 cwd），末项为用户级（`~/` 展开）；缺失的文件静默跳过 |

### 3.5 Session

| 字段 | 默认 | 说明 |
|------|------|------|
| `session.dir` | `.mycode/sessions` | 存储目录 |
...
`src/mycode/llm/providers.py` 提供下列 profile，`provider` 字段命中时作为默认值：
...
| 变量 | 用途 |
|------|------|
| `MYCODE_CONFIG` | 指定额外的 settings.json 路径 |
| `MYCODE_PROVIDER` | 覆盖 `provider` |
| `MYCODE_MODEL` | 覆盖 `model` |
| `MYCODE_BASE_URL` | 覆盖 `base_url` |
| `MYCODE_DEBUG` | `1` = 打开 debug（等价 `--debug`） |
| `<API_KEY_ENV>` | 各 provider 的 key（由 `api_key_env` 指定） |

**不提供** `MYCODE_API_KEY` 这种通用名——强制走 `api_key_env` 间接引用，避免 shell history 意外泄露。
...
| `--debug` | 详细日志 |
| `--debug-raw` | 不脱敏日志（仅写 `.mycode/debug/`） |
...
```
mycode: invalid config at .mycode/settings.json
