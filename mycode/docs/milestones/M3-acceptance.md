...
### 3.2 M3-2 退出总结写 `.mycode/MEMORY.md`
...
| M3-7.5 | wheel 含所有模块 | mycode/** 在 wheel 内 | `unzip -l dist/*.whl` | ✅ |
...
Successfully built dist/mycode-0.1.0.dev0.tar.gz
Successfully built dist/mycode-0.1.0.dev0-py3-none-any.whl
```

wheel 内容已确认包含所有 `mycode/**` 模块及子模块（team/ mcp/ memory/ session/ context/ ...）。

### 4.4 代码体积

```
src/mycode/mcp/client.py          +40 行（sse/http 分支）
src/mycode/memory/summarize.py    ≈ 100 行
src/mycode/team/bus.py            ≈ 110 行
src/mycode/team/manager.py        ≈ 120 行
src/mycode/team/loop.py           ≈ 300 行
src/mycode/team/tools.py          ≈ 180 行
src/mycode/team/protocol.py       ≈ 130 行
src/mycode/ui/repl.py             +~60 行（/team /inbox /exit-summary）
src/mycode/cli.py                 +~40 行（team/mcp 接线）
...
uv run mycode --list-sessions                       # 可列出 session,不触发 MCP

# 3. 退出总结（启动 REPL 聊几句后）
# mycode > 我喜欢中文回答
# mycode > 写代码前先读文件
# mycode > /exit-summary
# 应看到: ✓ Appended N lines to .mycode/MEMORY.md

cat .mycode/MEMORY.md                               # 查看追加的内容

# 4. 多 Agent（需要在 .mycode/settings.json 加 "team": {"enabled": true}）
# cat > .mycode/settings.json << 'EOF'
# {"team": {"enabled": true}}
# EOF
# uv run mycode
# mycode > 用 SpawnTeammate 起一个叫 alice 的 backend 队友,让她看下 README
# mycode > /team
# mycode > /inbox
