...
```
src/mycode/tools/ask_user.py     ≈ 140 行
src/mycode/tools/plan_mode.py    ≈ 130 行
src/mycode/tools/web.py          ≈ 100 行
src/mycode/tools/builtin.py      +80 行 (MultiEdit)
src/mycode/tools/background.py   +80 行 (kill + Popen 重构)
src/mycode/team/loop.py          +~5 行 (summarize_llm)
src/mycode/team/tools.py         +~5 行
src/mycode/agent/dispatcher.py   +20 行 (plan gate)
src/mycode/agent/loop.py         +~5 行
src/mycode/cli.py                +30 行 (接线)
src/mycode/ui/repl.py            +50 行 (_interactive_ask / /plan slash)
-----------------------------------------------
...
# 2. WebFetch
uv run mycode -p "用 WebFetch 拉 https://httpbin.org/html,告诉我内容结构"

# 3. Plan Mode
uv run mycode
# mycode > 进入 plan 模式,然后只看代码,给我一个重构 team/loop.py 的方案
# (agent 调 EnterPlanMode, Read, ExitPlanMode → 弹审批)

# 4. AskUserQuestion
uv run mycode
# mycode > 我让你选主模型,直接用 AskUserQuestion 让我从 3 个选一个

# 5. BackgroundKill
uv run mycode
# mycode > 用 BackgroundRun 跑 sleep 300,然后立刻用 BackgroundKill 把它杀掉
```
