...
from mycode.tools.worktree import (
    _load_state,
    _run_enter,
    _run_exit,
    _run_status,
...
def test_enter_creates_worktree_and_changes_root(tmp_path, monkeypatch):
    ...
    # 实际目录存在
    wt = tmp_path / ".mycode" / "worktrees" / "feature-x"
    assert wt.is_dir()
    # workspace_root 被改了
    ...
    assert state["branch"] == "mycode/feature-x"
...
def test_exit_keep_preserves_dir(tmp_path, monkeypatch):
    ...
    wt = tmp_path / ".mycode" / "worktrees" / "keep-me"
...
def test_exit_remove_clean_worktree(tmp_path, monkeypatch):
    ...
    wt = tmp_path / ".mycode" / "worktrees" / "rm-me"
...
def test_exit_remove_rejects_dirty(tmp_path, monkeypatch):
    ...
    wt = tmp_path / ".mycode" / "worktrees" / "dirty"
...
def test_exit_remove_with_discard(tmp_path, monkeypatch):
    ...
    wt = tmp_path / ".mycode" / "worktrees" / "force-rm"
    (wt / "new.txt").write_text("uncommitted\n")
