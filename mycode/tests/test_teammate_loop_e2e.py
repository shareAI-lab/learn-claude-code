...
from mycode.team.loop import start_teammate_loop
...
@pytest.mark.xfail(
    reason="时间敏感: 在跨文件的完整 pytest 套件里偶发失败(单跑稳定),M6 会重构",
    strict=False,
)
@respx.mock
def test_teammate_wakes_from_idle_on_inbox(tmp_path, monkeypatch):
    ...
    # 任意次 LLM 调用都返回 stop,让测试专注于 IDLE→唤醒→IDLE→SHUTDOWN 的生命周期
    # 避免 side_effect 列表用完后抛 StopIteration 导致 flaky
    respx.post("https://api.openai.com/v1/chat/completions").mock(
        return_value=httpx.Response(200, json=_make_completion("turn done"))
    )
...
def test_teammate_autoclaim_task(tmp_path, monkeypatch):
    """autonomous=True 时,idle 阶段应从 .mycode/tasks/ 认领未阻塞任务。"""
    cfg = _setup(tmp_path, monkeypatch)
    respx.post("https://api.openai.com/v1/chat/completions").mock(
