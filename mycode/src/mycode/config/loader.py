...
顺序（低 → 高）：
1. env defaults
2. ~/.mycode/settings.json
3. ./.mycode/settings.json
4. --config <path> / $MYCODE_CONFIG
...
def _load_json_file(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(f"mycode: invalid JSON in {path}: {e}")
...
def load_config(
    *,
    ...
    # 2. user settings
    user_settings = Path.home() / ".mycode" / "settings.json"
    layers.append(_load_json_file(user_settings))

    # 3. project settings
    project_settings = Path.cwd() / ".mycode" / "settings.json"
    layers.append(_load_json_file(project_settings))

    # 4. --config / MYCODE_CONFIG
