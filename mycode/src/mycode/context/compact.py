...
- microcompact: 每轮调用前,把超过 keep_recent_tool_results 的、
  且体积 > evict_threshold_bytes 的 tool_result 外置到 .mycode/blobs/<hash>.txt
...
def _blobs_dir(cfg: Config) -> Path:
    d = cfg.workspace_root() / ".mycode" / "blobs"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _transcripts_dir(cfg: Config) -> Path:
    d = cfg.workspace_root() / ".mycode" / "transcripts"
    d.mkdir(parents=True, exist_ok=True)
    return d
...
def auto_compact(
    ...
    完整 messages 先落盘到 .mycode/transcripts/。
    """
    # 落盘
    ts = int(time.time())
