...
def _maybe_resume_worktree(cfg) -> None:
    ...
    state_path = Path.cwd() / ".mycode" / "worktrees" / "state.json"
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="mycode", description="OpenAI-compatible coding agent")
    ...
    p.add_argument("--version", action="version", version=f"mycode {__version__}")
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not cfg.model:
        print("mycode: model is required (set via --model or provider profile)", file=sys.stderr)
        return 2
    ...
    if args.resume:
        ...
        if target == "latest":
            ...
            if not latest:
                print("mycode: no sessions to resume", file=sys.stderr)
                return 2
            target = latest
        try:
            ...
            print(f"mycode: {e}", file=sys.stderr)
            return 2
    else:
