...
baseline; these files show the same s01-s06 lessons through Deep Agents's
...
| Original baseline | Deep Agents comparison | Lesson |
|---|---|---|
| `agents/s01_agent_loop.py` | `agents_deepagents/s01_agent_loop.py` | Deep Agents owns the repeated model/tool loop; the harness owns history and CLI boundaries. |
...
| `agents/s04_subagent.py` | `agents_deepagents/s04_subagent.py` | Spawn a child Deep Agents agent with fresh messages and return only a summary. |
