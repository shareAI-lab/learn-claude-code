...
s15 の inbox ポーリングは**デュアルモードアーキテクチャ**を採用：

### システム側自動注入
`agent_loop` は各反復の終了時に自動的に `BUS.read_inbox("lead")` を呼び出し、未読メッセージを `[Inbox]` ユーザーメッセージとして `history` に注入。これにより、LLM が `check_inbox` ツールを能動的に呼び出さなくても、チームメイトのメッセージが失われないことを保証。

### LLM 側能動クエリ
`check_inbox` ツールは LLM が呼び出し可能なツールとして保持、オンデマンドで inbox 内容を取得。LLM がツール呼び出しチェーン内でメッセージを確認する必要がある場面に適用。

### 設計のトレードオフ
| モード | 利点 | 欠点 |
|--------|------|------|
| 自動注入 | メッセージ損失なし、LLM の判断不要 | コンテキストウィンドウを消費 |
| 能動クエリ | オンデマンド取得、コンテキスト節約 | LLM の判断に依存、見落としの可能性 |

s15 は**デュアルモード共存**を選択：自動注入をセーフティネット、能動クエリを補完として。これは s14 の cron_queue や s13 の background_results の設計と一貫 — システム側が重要イベントを能動的にプッシュし、LLM 側は細粒度クエリ用のツールを保持。

### Issue #413 との関係
[Issue #413](https://github.com/shareAI-lab/learn-claude-code/issues/413) は check_inbox を完全にシステム側に任せるべきかを提起。現在の設計はデュアルモードを維持：
1. 自動注入されるメッセージは最初の 200 文字に切り詰められる、LLM の能動クエリで完全な内容を取得可能
2. 一部のシナリオでは、LLM が各ラウンド末尾ではなく特定のツール呼び出し後に inbox を確認する必要がある
3. 教学目的：システム側と LLM 側の協調の二つのモードを示す
