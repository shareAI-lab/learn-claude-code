...
def test_read_write_edit_roundtrip(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert write_file.invoke({"path": "notes.txt", "content": "alpha\nbeta\n"}) == "Wrote 11 bytes to notes.txt"
    assert read_file.invoke({"path": "notes.txt"}) == "alpha\nbeta"
    assert read_file.invoke({"path": "notes.txt", "limit": 1}) == "alpha\n... (1 more lines)"
    assert edit_file.invoke({"path": "notes.txt", "old_text": "beta", "new_text": "gamma"}) == "Edited notes.txt"
    assert read_file.invoke({"path": "notes.txt"}) == "alpha\ngamma"


def test_bash_blocks_dangerous_commands() -> None:
    assert bash.invoke({"command": "rm -rf /"}) == "Error: Dangerous command blocked"
