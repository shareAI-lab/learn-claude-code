...
Env/client setup and the base tool schemas come from common.py. s18's base
tools gain a ``cwd`` parameter (worktree isolation), so they are re-defined
locally and delegate to make_base_tools(cwd or WORKDIR). The __main__ REPL
stays inline because the lead inbox routing needs a custom post-turn step.
"""
...
def _base_tools(cwd: Path = None):
    """Base tool closures bound to cwd (or WORKDIR) — s18's worktree isolation
    dispatches tool calls with a worktree-specific cwd."""
    return make_base_tools(cwd or WORKDIR)

# ── Task System (base from s12 via mechanisms/tasks.py; claim_task enhanced — s17 teaches owner-check) ──
# s12 defines the base inline; s18-s20 import it and override claim_task
# with the s17 owner-check + missing-deps enhancement. The optional
# `worktree` slot on Task is forward-compatible (s12-s17 leave it None).
from mechanisms.tasks import (
    init_tasks, Task, create_task, save_task, load_task, list_tasks,
    get_task_json, can_start, complete_task, task_path)
init_tasks(WORKDIR)
